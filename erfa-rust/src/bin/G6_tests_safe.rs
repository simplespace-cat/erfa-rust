...
fn t_bi00(status: &mut i32) {
    let result = G6_safe::eraBi00_safe();
    match result {
        Ok((dpsibi, depsbi, dra)) => {
            vvd(
                dpsibi,
        ...
        Err(_) => {
            *status = 1;
            println!("eraBi00_safe failed: unexpected error");
        }
    }
}

fn t_bp00(status: &mut i32) {
    let result = G6_safe::eraBp00_safe(2400000.5, 50123.9999);
    match result {
        Ok((rb, rp, rbp)) => {
            vvd(
                rb[0][0],
        ...
        Err(_) => {
            *status = 1;
            println!("eraBp00_safe failed: unexpected error");
        }
    }
}

fn t_bp06(status: &mut i32) {
    let result = G6_safe::eraBp06_safe(2400000.5, 50123.9999);
    match result {
        Ok((rb, rp, rbp)) => {
            vvd(
                rb[0][0],
        ...
        Err(_) => {
            *status = 1;
            println!("eraBp06_safe failed: unexpected error");
        }
    }
}
...
fn t_bpn2xy(status: &mut i32) {
    ...
    let result = G6_safe::eraBpn2xy_safe(&rbpn);
    match result {
        Ok((x, y)) => {
            vvd(
                x,
        ...
        Err(_) => {
            *status = 1;
            println!("eraBpn2xy_safe failed: unexpected error");
        }
    }
}
