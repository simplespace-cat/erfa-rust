...
fn t_refco(status: &mut i32) {
    ...
    let (refa, refb) = eraRefco_safe(phpa, tc, rh, wl).unwrap();
...
fn t_rxpv(status: &mut i32) {
    ...
    let rpv = eraRxpv_safe(&r, &pv).unwrap();
...
fn t_rxr(status: &mut i32) {
    ...
    let atb = eraRxr_safe(&a, &b).unwrap();
