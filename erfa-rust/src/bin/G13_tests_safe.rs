...
fn t_epv00(status: &mut i32) {
    let result = G13_safe::eraEpv00_safe(2400000.5, 53411.52501161);
    ...
    match result {
        Ok((pvh, pvb, j)) => {
            ...
            viv(j, 0, "eraEpv00_safe", "j", status);
        }
        Err(_) => {
            *status = 1;
            println!("eraEpv00_safe failed: unexpected error");
        }
    }
}
