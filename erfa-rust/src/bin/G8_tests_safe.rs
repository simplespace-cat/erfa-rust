...
fn t_c2tcio(status: &mut i32) {
    ...
    let result = G8_safe::eraC2tcio_safe(&rc2i, era, &rpom, &mut rc2t);
    ...
    match result {
        ...
        Err(_) => {
            *status = 1;
            println!("eraC2tcio_safe failed: unexpected error");
        }
    }
}
...
fn t_c2teqx(status: &mut i32) {
    ...
    let result = G8_safe::eraC2teqx_safe(&rbpn, gst, &rpom, &mut rc2t);
    ...
    match result {
        ...
        Err(_) => {
            *status = 1;
            println!("eraC2teqx_safe failed: unexpected error");
        }
    }
}
...
fn t_c2tpe(status: &mut i32) {
    ...
    let result = G8_safe::eraC2tpe_safe(tta, ttb, uta, utb, dpsi, deps, xp, yp, &mut rc2t);
    ...
    match result {
        ...
        Err(_) => {
            *status = 1;
            println!("eraC2tpe_safe failed: unexpected error");
        }
    }
}
...
fn t_c2txy(status: &mut i32) {
    ...
    let result = G8_safe::eraC2txy_safe(tta, ttb, uta, utb, x, y, xp, yp, &mut rc2t);
    ...
    match result {
        ...
        Err(_) => {
            *status = 1;
            println!("eraC2txy_safe failed: unexpected error");
        }
    }
}

fn t_cal2jd(status: &mut i32) {
    let result = G8_safe::eraCal2jd_safe(2003, 6, 1);

    match result {
        Ok(((djm0, djm), j)) => {
            vvd(djm0, 2400000.5, 0.0, "eraCal2jd_safe", "djm0", status);
            vvd(djm, 52791.0, 0.0, "eraCal2jd_safe", "djm", status);
            viv(j, 0, "eraCal2jd_safe", "j", status);
        }
        Err(_) => {
            *status = 1;
            println!("eraCal2jd_safe failed: unexpected error");
        }
    }
}
...
fn t_cp(status: &mut i32) {
    ...
    let result = G8_safe::eraCp_safe(&p, &mut c);

    match result {
        Ok(()) => {
            vvd(c[0], 0.3, 0.0, "eraCp_safe", "1", status);
            vvd(c[1], 1.2, 0.0, "eraCp_safe", "2", status);
            vvd(c[2], -2.5, 0.0, "eraCp_safe", "3", status);
        }
        Err(_) => {
            *status = 1;
            println!("eraCp_safe failed: unexpected error");
        }
    }
}
...
fn t_cpv(status: &mut i32) {
    ...
    let result = G8_safe::eraCpv_safe(&pv, &mut c);

    match result {
        Ok(()) => {
            vvd(c[0][0], 0.3, 0.0, "eraCpv_safe", "p1", status);
            vvd(c[0][1], 1.2, 0.0, "eraCpv_safe", "p2", status);
            vvd(c[0][2], -2.5, 0.0, "eraCpv_safe", "p3", status);
            vvd(c[1][0], -0.5, 0.0, "eraCpv_safe", "v1", status);
            vvd(c[1][1], 3.1, 0.0, "eraCpv_safe", "v2", status);
            vvd(c[1][2], 0.9, 0.0, "eraCpv_safe", "v3", status);
        }
        Err(_) => {
            *status = 1;
            println!("eraCpv_safe failed: unexpected error");
        }
    }
}
...
fn t_cr(status: &mut i32) {
    ...
    let result = G8_safe::eraCr_safe(&r, &mut c);

    match result {
        Ok(()) => {
            vvd(c[0][0], 2.0, 0.0, "eraCr_safe", "11", status);
            vvd(c[0][1], 3.0, 0.0, "eraCr_safe", "12", status);
            vvd(c[0][2], 2.0, 0.0, "eraCr_safe", "13", status);
            vvd(c[1][0], 3.0, 0.0, "eraCr_safe", "21", status);
            vvd(c[1][1], 2.0, 0.0, "eraCr_safe", "22", status);
            vvd(c[1][2], 3.0, 0.0, "eraCr_safe", "23", status);
            vvd(c[2][0], 3.0, 0.0, "eraCr_safe", "31", status);
            vvd(c[2][1], 4.0, 0.0, "eraCr_safe", "32", status);
            vvd(c[2][2], 5.0, 0.0, "eraCr_safe", "33", status);
        }
        Err(_) => {
            *status = 1;
            println!("eraCr_safe failed: unexpected error");
        }
    }
}
