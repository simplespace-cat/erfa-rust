...
fn t_obl06(status: &mut i32) {
    let result = unsafe { H1::eraObl06(2400000.5, 54388.0) };
    vvd(result, 0.4090749229387258204, 1e-14, "eraObl06", "", status);
}

fn t_obl80(status: &mut i32) {
    let result = unsafe { H1::eraObl80(2400000.5, 54388.0) };
    vvd(result, 0.4090751347643816218, 1e-14, "eraObl80", "", status);
}
