...
fn t_atci13(status: &mut i32) {
    ...
    unsafe {
        H1::eraAtci13(
            rc, dc, pr, pd, px, rv, date1, date2, &mut ri, &mut di, &mut eo,
        );
    }

    vvd(ri, 2.710121572968696744, 1e-12, "eraAtci13", "ri", status);
    vvd(di, 0.1729371367219539137, 1e-12, "eraAtci13", "di", status);
    vvd(
        eo,
...
fn t_atciq(status: &mut i32) {
    ...
    unsafe {
        ...
        H1::eraAtciq(rc, dc, pr, pd, px, rv, &mut astrom, &mut ri, &mut di);
    }
...
fn t_atciqn(status: &mut i32) {
    let mut b: [H1::eraLDBODY; 3] = unsafe { std::mem::zeroed() };
    let date1 = 2456165.5;
    let date2 = 0.401182685;
    let mut eo = 0.0;
    let rc = 2.71;
...
fn t_atciqz(status: &mut i32) {
    ...
    unsafe {
        H1::eraApci13(date1, date2, &mut astrom, &mut eo);
        H1::eraAtciqz(rc, dc, &mut astrom, &mut ri, &mut di);
    }
...
fn t_atic13(status: &mut i32) {
    ...
    unsafe {
        H1::eraAtic13(ri, di, date1, date2, &mut rc, &mut dc, &mut eo);
    }
...
fn t_aticq(status: &mut i32) {
    ...
    unsafe {
        H1::eraApci13(date1, date2, &mut astrom, &mut eo);
        H1::eraAticq(ri, di, &mut astrom, &mut rc, &mut dc);
    }
...
fn t_aticqn(status: &mut i32) {
    ...
    unsafe {
        H1::eraAticqn(ri, di, &mut astrom, 3, b.as_mut_ptr(), &mut rc, &mut dc);
    }

    vvd(rc, 2.709999575033027333, 1e-12, "eraAticqn", "rc", status);
    vvd(dc, 0.1739999656316469990, 1e-12, "eraAticqn", "dc", status);
}
...
fn t_atio13(status: &mut i32) {
    ...
    let j = unsafe {
        H1::eraAtio13(
            ri, di, utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl, &mut aob, &mut zob,
